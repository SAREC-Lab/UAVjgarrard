...
class SimpleGCS:
    ...
    def registerDrone(self, home,name,virtual=True):
        ...
        time.sleep(1)
        handshake = dronology_utils.DroneHandshakeMessage.from_vehicle(vehicle, self.dronology._g_id,name)
    ...
    def connect(self):
        self.dronology = dronology_utils.Connection(None, "localhost", 1234,self.g_id)
    ...
    def connect_physical_vehicle(self, home):
        ...
        vehicle.wait_ready(timeout=300)
    ...
    while DO_CONT:
       # for i, v in enumerate(vehicles):
        for name, v in vehicles.iteritems():
            state = dronology_utils.StateMessage.from_vehicle(v,dronology._g_id,name)
            state_str = str(state)
